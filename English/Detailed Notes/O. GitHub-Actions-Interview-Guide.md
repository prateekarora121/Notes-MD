...
11. **Where Runner Minutes Actually Go** / **Workflow-Level Timeout Hygiene** — cost/performance optimization wasn't discussed at all in the source, and "reduce our CI bill" is a common lead-style question.
12. **GitHub Actions vs Azure DevOps Pipelines** — highly relevant to this audience (.NET shops frequently run or migrate from Azure DevOps); not present in source.

**Contradictions/staleness flagged inline** (not conflicting notes per se, but factual staleness in the source that would be wrong if repeated verbatim in 2026):
- Action versions throughout the source (`checkout@v3`, `cache@v3`, `upload-artifact@v3`/`download-artifact@v3`) are outdated/deprecated — updated to v4+ throughout this guide, with an explicit callout under Core Concepts and Caching & Artifacts.
- Runner OS image list (`ubuntu-20.04`, `windows-2019`, `macos-12`) references retired images — updated list provided, flagged to verify against GitHub's live support matrix since `-latest` rotates.
- `actions/setup-debugging`, cited in the source as the debugging tool, does not appear to be a real official action — flagged as likely inaccurate in the source, with the correct current approach (`ACTIONS_STEP_DEBUG`/`ACTIONS_RUNNER_DEBUG` secrets, `tmate`) substituted.

## Summary of `[gaps]` Additions (This Pass)

A follow-up gap-analysis review specifically flagged that this guide's cloud/deployment examples defaulted to generic or Azure-flavored patterns without tying back to AWS (the candidate's actual target cloud) or Terraform/CDKTF (the candidate's actual IaC tooling). One `[gaps]`-tagged section was added to close this:

1. **AWS-Native CI/CD Integration Points** — the guide already covered OIDC federation conceptually and showed a bare-bones `aws-actions/configure-aws-credentials` snippet, but didn't go deep on the AWS-specific mechanics an interviewer would actually probe: the full `configure-aws-credentials` flow (STS `AssumeRoleWithWebIdentity` under the hood), the exact IAM OIDC provider + trust policy shape (including a Terraform `aws_iam_openid_connect_provider`/`aws_iam_role` example, since Terraform is the candidate's real IaC tool — not just a conceptual mention), and — the part that was entirely missing — how caching and artifacts concretely connect to deploying to **ECS** (image-based, `type=gha` layer caching through to ECR, task-definition rollout) versus **Lambda** (artifact/zip-based, `upload-artifact` as the deployable handoff). This matters because "OIDC federation" alone is a general-cloud talking point; being able to name the specific AWS action, the trust-policy claim conditions (`aud`/`sub`), and which of cache-vs-artifact fits which AWS compute target is what actually signals AWS-specific (not generic-cloud) fluency in an interview.

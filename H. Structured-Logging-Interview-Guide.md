...
1. **Distributed tracing** — the Gateway issues a W3C `traceparent` (`TraceId = T123`); every downstream service captures it via ASP.NET Core's built-in `Activity`/`DiagnosticSourc
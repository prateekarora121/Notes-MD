...
The sections above are already senior/lead-calibrated on paper. The additions below come from a formal gap-analysis review of this guide against 2026 senior .NET/AWS system-design interview expectations — they close specific, named gaps rather than re-covering ground already handled well.

### Expanding the Worked-Example Repertoire [gaps]

Three worked examples (URL Shortener, Rate Limiter, Notification System) is a good start, but senior/staff loops draw from a much larger pool. Practice these six the same way — requirements → estimation → high-level design → deep dive → trade-offs — using the framework already established above:

- **Design a News Feed (Twitter/Instagram-style).** Core tension: fan-out-on-write (precompute each follower's feed on every post — fast reads, expensive/slow for celebrities with millions of followers) vs fan-out-on-read (assemble the feed at request time by querying who you follow — cheap writes, expensive reads). Senior answer: hybrid — fan-out-on-write for normal users, fan-out-on-read (or a separate "celebrity" path) for high-follower accounts, because a single strategy breaks at the tails.
- **Design a Chat System (WhatsApp-style).** Core components: WebSocket/long-lived connections for real-time delivery, a presence service (online/offline/typing), message persistence with delivery guarantees (sent/delivered/read receipts), and fan-out for group chats. Key deep-dive: how do you route a message to a recipient who could be connected to any of thousands of stateless connection-handling servers? (Answer: a connection-registry/lookup service — e.g., Redis mapping `userId -> serverId` — so any server can find where to forward the message.)
- **Design a Ride-Sharing Dispatch System (Uber-style).** Core problem: geospatial matching at scale — finding the nearest available driver to a rider. Senior answer: geohashing or a quadtree to index driver locations so "find nearby drivers" is a fast spatial query instead of a full scan; frequent location updates (every few seconds per driver) mean the location store needs to be optimized for high-write-throughput, not strong consistency.
- **Design a Distributed Key-Value Store** (the "build a mini-DynamoDB" prompt). Ties directly into content already in this guide: consistent hashing for partitioning, replication factor for durability, read/write quorums (`R`/`W`) for tunable consistency, and vector clocks or last-write-wins for conflict resolution on concurrent writes to the same key.
- **Design a Video Streaming Service (Netflix-style).** Core components: chunked video encoding at multiple bitrates, a CDN for edge delivery (this is where your caching-strategy content directly applies), adaptive bitrate streaming (client picks the next chunk's quality based on measured bandwidth), and a separate metadata/recommendation service decoupled from the video-serving path.
- **Design an E-Commerce Checkout/Inventory System.** Core problem: preventing overselling under concurrent checkouts. Senior answer: this is a direct application of the Sagas and Idempotency Keys content already in this guide — reserve inventory (with a TTL, in case checkout abandons), charge payment, confirm reservation; use compensating actions (release the reservation) if any step fails, and idempotency keys so a retried checkout request doesn't double-charge or double-reserve.

### Mapping Patterns to AWS Managed Services [gaps]

Every generic building block covered earlier in this guide has a concrete AWS managed-service equivalent. Given the target role is explicitly AWS-cloud, you should be able to translate instantly in both directions — if an interviewer says "how would you build this on AWS specifically," don't restate the generic pattern, name the service:

| Generic pattern (covered above) | AWS managed-service equivalent | Notes |
|---|---|---|
| Distributed cache (Redis, cache-aside) | **ElastiCache** (Redis or Memcached) | Same cache-aside/write-through patterns apply; ElastiCache just removes the operational burden of running Redis yourself |
| Message queue (point-to-point) | **SQS** (Simple Queue Service) | At-least-once delivery by default — the idempotency-key content above applies directly; supports dead-letter queues natively |
| Pub/Sub (topic, fan-out) | **SNS** (Simple Notification Service), often SNS → multiple SQS queues | SNS fans out to many subscribers; combine with SQS per-subscriber for durable, independently-scalable consumers |
| CDN / edge caching | **CloudFront** | Fronts S3 (static assets) or an ALB/API Gateway (dynamic content); this is the "cache at the layer closest to the client" principle from Best Practices above, taken to its AWS-native conclusion |
| Load balancer (L7) | **ALB** (Application Load Balancer) | Path/header-based routing — the AWS-native tool for the Strangler Fig gateway-routing pattern covered above |
| Load balancer (L4) | **NLB** (Network Load Balancer) | Ultra-low-latency, protocol-agnostic — used when you need raw TCP/UDP performance rather than HTTP-aware routing |
| Read replicas / horizontal read scaling | **RDS Read Replicas** or **Aurora Replicas** | Same eventual-consistency caveat from the CQRS+BFF reference architecture above applies directly |
| Orchestrated Sagas / workflow coordination | **Step Functions** | A managed alternative to hand-rolling a saga orchestrator — state machine defines each step and its compensating action declaratively |
| Event streaming (Kafka-equivalent) | **Kinesis Data Streams** (or MSK — Managed Streaming for Kafka, if you specifically need Kafka compatibility) | Kinesis is AWS-native with simpler ops; MSK is the drop-in Kafka option when you need Kafka's exact semantics/tooling |
| Serverless compute for background/event-driven work | **Lambda** | Already your strongest hands-on area — the Outbox Pattern's polling publisher is a natural Lambda-on-a-schedule (EventBridge Scheduler) job |
| Rate limiter's shared counter store | **ElastiCache (Redis)**, same as the distributed-cache row | The guide's own "what if 10 pods behind a load balancer" follow-up above is answered the same way on AWS: a shared Redis (ElastiCache), not per-instance memory |

### Running the Interview: Time-Boxing & Whiteboard Mechanics [gaps]

Knowing the patterns and running a good 45-60 minute live interview are different skills — this section is about the second one specifically, since it's the part that separates "read the notes" from "performed well in the room."

**Typical time-box for a 45-minute system design interview:**

| Phase | Time | What you're doing |
|---|---|---|
| Requirements clarification | ~5 min | Ask about scale (DAU/RPS), read:write ratio, consistency needs, latency SLAs — don't skip this even under time pressure; designing the wrong thing fast is worse than designing the right thing slightly slower |
| Back-of-envelope estimation | ~5 min | Use the capacity-estimation method from Core Concepts above — state assumptions out loud as you go |
| High-level architecture | ~15-20 min | Draw boxes and arrows; narrate why each component exists as you add it, don't just draw silently |
| Deep dive on 1-2 hard components | ~15-20 min | The interviewer will usually steer you here ("how exactly does X work") — this is where the detailed pattern content earlier in this guide gets used |
| Trade-offs / failure modes / wrap-up | ~5-10 min | Proactively name what you'd do differently at 10x scale, and at least one failure mode you haven't addressed yet |

**Whiteboard/diagramming fluency:** practice sketching the architectures in this guide (CQRS+BFF, Strangler Fig, Saga sequence diagrams) using whatever tool you'll actually be given — most remote interviews use a shared virtual whiteboard (Excalidraw, Miro, or a plain collaborative doc) or a coding-interview platform's built-in diagram pane (CoderPad, HackerRank). Being fluent with boxes/arrows/labels in that specific tool, at speed, while talking, is a distinct skill from knowing the architecture — rehearse it, don't assume it transfers from reading.

**Narrate continuously.** Silence while you think is the single most common reason a technically-correct design still reads as "junior" in feedback. Say what you're considering and why you're rejecting alternatives, even briefly ("I could shard by user ID, but I'll shard by tenant ID instead since it keeps each tenant's data — and transactions — on one shard").

### Multi-Tenant Data Isolation & PII Handling [gaps]

This is a system-design angle worth building into a strong personal story, since it maps directly onto real experience: a Dealership Management System handling multiple partner dealers' data, with state/county-specific business rules and compliance workflows, is inherently a multi-tenant, PII-adjacent system.

**Tenant isolation models** (the architectural decision, distinct from the EF Core-level global-query-filter mechanism covered in the EF Core guide):

| Model | Isolation | Cost/Complexity | When it fits |
|---|---|---|---|
| Silo (DB-per-tenant) | Strongest — separate database per tenant | Highest operational overhead (migrations, scaling, backups multiply per tenant) | Regulatory requirements demand hard isolation (e.g., a dealer/regulator requires their data never shares infrastructure), or tenants have wildly different scale |
| Pooled with tenant-key partitioning (shared DB, `TenantId` column + row-level filtering) | Logical isolation, enforced in application/ORM code | Lowest overhead, but a missed filter is a real data-leak risk | Most SaaS platforms at moderate-to-large scale — matches this guide's existing sharding-by-tenant-ID recommendation for the "shard a multi-tenant SaaS DB" question above |
| Bridge (schema-per-tenant) | Middle ground — same DB instance, separate schema per tenant | Moderate — easier backup/restore per tenant than pooled, cheaper than full silo | A middle number of tenants where per-tenant customization (schema differences) matters but full silo isn't justified |

**PII/compliance angle to raise proactively:** encryption at rest (AWS: RDS/DynamoDB encryption, KMS-managed keys) and in transit (TLS everywhere), field-level encryption or tokenization for the most sensitive fields (e.g., a dealer's financial/customer data) rather than relying on database-level encryption alone, and audit logging of who accessed which tenant's data and when — this last point ties directly into the Structured Logging guide's content and is frequently the first thing a compliance-minded interviewer asks about once multi-tenancy comes up.

**Cross-tenant leak as the #1 failure mode to name explicitly:** whichever isolation model you pick, the interview-winning statement is naming the specific mechanism that prevents tenant A's request from ever returning tenant B's data — a global query filter with a fail-closed default (queries throw rather than silently return unfiltered data if the tenant context isn't set), not just "we add a WHERE clause."
...
This pass added four sections under [Closing the Gap: Additional Prep](#closing-the-gap-additional-prep-gaps), based on a formal gap-analysis review of this guide against 2026 senior .NET/AWS system-design interview expectations. Unlike the `[new content]` pass above (which filled missing *knowledge*), this pass targets the gap between having strong written notes and performing well live — plus a few genuinely missing content items:

- **Expanding the Worked-Example Repertoire** — three worked examples wasn't enough breadth for staff/senior loops; added six more (news feed, chat, ride-sharing, distributed KV store, video streaming, e-commerce checkout) with the specific hard trade-off each one tests.
- **Mapping Patterns to AWS Managed Services** — the guide taught every pattern generically (cache, queue, CDN, load balancer) without ever naming the AWS service that implements it. Given the target role is explicitly AWS-cloud, this was the single highest-leverage addition — it doubles as AWS interview prep, not just system design prep.
- **Running the Interview: Time-Boxing & Whiteboard Mechanics** — the guide had zero content on the *performance* skill of running a live 45-60 minute interview (time allocation per phase, narrating out loud, whiteboard-tool fluency) as distinct from knowing the patterns. This is the most likely reason a candidate with genuinely strong notes still reads as "intermediate" in a live loop.
- **Multi-Tenant Data Isolation & PII Handling** — added because it maps directly onto real hands-on experience (a multi-tenant Dealership Management System with compliance workflows) that wasn't yet turned into a system-design talking point; also closes a gap flagged separately in the AWS and Microservices guides around compliance/security depth.

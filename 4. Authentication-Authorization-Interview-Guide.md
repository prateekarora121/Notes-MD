...
Not present in the source notes at all. This is one of the highest-signal gaps to close going into 2025/2026 senior interviews: passwordless auth built on WebAuthn/FIDO2 ("passkeys") has moved from "emerging standard" to "default recommendation" across major platforms (Apple, Google, Microsoft) and increasingly enterprise IdPs (Azure AD/Entra ID, Okta). Interviewers use it to probe whether a candidate's security knowledge has kept pace past "just use MFA/OTP."

**The core cryptographic idea — no shared secret to steal:**

Unlike a password or OTP (both of which are *shared secrets* — the same value exists on both the client and the server, or is derived from something both sides know), WebAuthn is built on **asymmetric key pairs generated per device, per site**:

- On registration, the authenticator (a platform authenticator like Windows Hello/Touch ID, or a roaming one like a hardware security key) generates a **new public/private key pair scoped to that origin** (e.g., `app.example.com`).
- The **private key never leaves the authenticator** — it's typically sealed in a secure enclave/TPM and is not exportable, not even to the user.
- The **public key is sent to and stored by the server**, associated with the user's account and a `credentialId`.
- Because each key pair is bound to a single origin, the same authenticator produces an entirely different key pair for a different site — there is nothing reusable across sites, unlike a password a user might reuse.

**Why this is phishing-resistant in a way passwords and OTP fundamentally are not:**

- A password or OTP is just a string — it can be typed (or auto-filled) into *any* page that asks for it, including a convincing phishing clone of the real site. The user (or a compromised autofill) cannot reliably tell `example.com` apart from `examp1e.com`.
- A WebAuthn credential is **cryptographically bound to the origin** by the browser/OS itself, not by the user's judgment. When a site invokes the WebAuthn API, the browser includes the actual origin in the data that gets signed, and the authenticator will only produce a valid signature for the origin the credential was registered against. A phishing site at a different origin literally cannot obtain a valid signed assertion — there's no "trick the user into typing it in" attack surface, because the user never sees or types anything secret at all.
- This is the key interview differentiator from OTP-based MFA: OTP still relies on the user (or their browser) correctly identifying the legitimate site before entering the code — real-time phishing proxies (e.g., adversary-in-the-middle kits) defeat OTP-based MFA routinely. WebAuthn removes that human trust step entirely.

**Registration and authentication ceremonies, conceptually:**

1. **Registration ("attestation") ceremony:**
   - Server generates a random **challenge** and sends it to the browser along with relying-party (RP) info (your app's origin) and user info.
   - Browser calls `navigator.credentials.create()`, which invokes the platform authenticator.
   - The authenticator generates a new key pair, signs the challenge with the new private key, and returns the public key + `credentialId` + attestation data to the browser, which forwards it to the server.
   - Server verifies the attestation/signature and **stores the public key + `credentialId`** against the user's account. The private key stays on the device permanently.
2. **Authentication ("assertion") ceremony:**
   - Server generates a new random challenge and sends it (optionally scoped to the user's registered `credentialId`s).
   - Browser calls `navigator.credentials.get()`; the authenticator asks the user for a local gesture (biometric/PIN/security key tap) and, if successful, **signs the challenge with the stored private key**.
   - Server looks up the stored public key for that `credentialId` and verifies the signature against the challenge it issued. If valid, the user is authenticated — no password or OTP ever transmitted.
...
**How this fits into an ASP.NET Core app:**

- ASP.NET Core Identity has been adding native passkey/WebAuthn support in recent .NET releases, letting a user register a passkey as an additional (or primary) credential alongside a password (verify against current ASP.NET Core version — this surface has been actively evolving and specifics depend on the exact release you're targeting).
- Before/independent of that native support, the long-standing practical option in .NET is the community library **`Fido2NetLib`**, which implements the server-side WebAuthn ceremonies (challenge generation, attestation/assertion verification, credential storage helpers) that you wire up behind your own API endpoints, with the browser-side `navigator.credentials` calls done in JS/TypeScript.
- Regardless of library, the architectural shape is always: your server acts as the **Relying Party**, storing public keys + credential IDs (not passwords) and issuing your normal session/JWT once the WebAuthn ceremony succeeds — WebAuthn replaces the *credential-verification* step, not your existing session/token issuance downstream of login.

**Interviewer follow-up:** *"Does this replace MFA?"* — WebAuthn/passkeys are often described as inherently multi-factor: something you have (the device/authenticator, bound cryptographically) plus something you are/know (the biometric or PIN gate that unlocks the local private key). Many organizations treat a passkey login as satisfying MFA on its own, though policy varies — worth stating explicitly that this is a security-posture decision, not a technical requirement, and should be marked as "verify against current ASP.NET Core version"/org policy when giving a definitive answer.

### 3.13 DPoP (RFC 9449 — Demonstrating Proof of Possession) [gaps]

Not present in the source notes. DPoP is a newer OAuth extension (finalized as RFC 9449) that senior interviewers are increasingly asking about because it directly addresses the best-known weakness of bearer tokens discussed elsewhere in this guide (see [4.2](#42-new-content-opaque-tokens-vs-jwt-for-revocation--the-real-trade-off)) — and it's a lighter-weight alternative to full mTLS (see [4.5](#45-new-content-mtls-for-service-to-service-auth)) for solving a related problem.

**The problem: bearer tokens are, definitionally, bearer instruments.**

Every access token discussed elsewhere in this guide — JWT or opaque — is a **bearer token**: whoever holds it (bears it) can use it. The server has no way to distinguish the legitimate client from an attacker who has stolen the token (via XSS, a compromised log, a leaked proxy, a malicious browser extension, etc.). A stolen bearer token can be replayed from any IP, any device, any client, and the resource server cannot tell the difference — this is precisely why revocation and short expiry matter so much for bearer tokens (as covered in [4.1](#41-why-jwt-revocation-is-hard--and-the-real-strategies)), because detection/response, not prevention, is the only lever available once a bearer token leaks.

**How DPoP binds a token to a specific client:**

DPoP ("Demonstrating Proof of Possession") adds a cryptographic binding layer on top of OAuth so that a token is only usable by the client that originally requested it, even if the token itself is stolen:

1. The client generates its own **public/private key pair** (held locally, e.g., in browser `CryptoKey`/non-extractable storage or a secure module — never sent to the server).
2. For **every** HTTP request (both the token request and later API calls), the client creates a **DPoP proof**: a small, short-lived JWT that includes the HTTP method, the target URL, a timestamp, and a unique identifier (`jti`) — and signs it with its private key.
3. When requesting the access token, the client includes this DPoP proof; the authorization server captures a **hash of the client's public key** and embeds it inside the issued access token (as a claim, conceptually `cnf.jkt` — the "confirmation" / "JWK thumbprint" claim).
4. On every subsequent API call, the client sends both the **access token** and a **fresh DPoP proof** (signed for that specific request's method/URL/timestamp) in an `Authorization: DPoP <token>` header plus a `DPoP` header carrying the proof.
5. The resource server verifies **two independent things**: (a) the DPoP proof's signature is valid and matches the request's method/URL/recency window, and (b) the public key hash in that proof matches the `cnf.jkt` thumbprint embedded in the access token. Only if both check out is the token accepted.

A stolen access token alone is now useless to an attacker — without the corresponding private key (which never left the legitimate client), they cannot produce a valid DPoP proof, so the key-hash check fails on the resource server.

| | Plain Bearer Token | DPoP-Bound Token |
|---|---|---|
| What's presented per request | Just the access token | Access token **+** a freshly signed DPoP proof JWT |
| What a thief needs to replay it | Just the token itself | The token **and** the client's private key (never transmitted, so effectively unobtainable from a token leak alone) |
| Server-side check | Signature/`iss`/`aud`/`exp` on the token only | Token checks **plus** DPoP proof signature **plus** public-key-hash match (`cnf.jkt`) |
| Infrastructure required | Standard OAuth token issuance | Client-side key generation/storage + per-request proof signing; server-side proof verification + replay cache for `jti` |
| Protects against | Nothing extra beyond normal token expiry | Bearer-token theft/replay from a different client (XSS token exfiltration, log leakage, etc.) |
...
**DPoP vs mTLS — why DPoP is the "lighter-weight" option:**

- **mTLS** (see [4.5](#45-new-content-mtls-for-service-to-service-auth)) binds a token/connection to a client via a full X.509 certificate exchanged during the TLS handshake — strong, but it requires certificate issuance, distribution, rotation, and typically a service mesh or gateway to make operationally sane. It's transport-layer proof of possession.
- **DPoP** achieves a similar *application-layer* goal (proving the caller holds a specific private key) using ordinary application-level JWTs and standard public-key crypto the client can generate itself at runtime — no CA, no certificate lifecycle, no changes to the TLS handshake or infrastructure. This makes it far more practical for public clients like SPAs and mobile apps, where provisioning a client certificate is impractical.
- **Trade-off:** DPoP protects the *token*, not the *transport connection* — it doesn't authenticate the TLS channel itself the way mTLS does, and it adds a small amount of per-request overhead (generating/signing a proof, and the resource server needs a short-lived replay cache keyed on `jti` to prevent proof replay within its validity window). In practice, DPoP and mTLS solve overlapping-but-distinct problems and are chosen based on client type: mTLS suits service-to-service/backend contexts that already have certificate infrastructure; DPoP suits browser/mobile/public-client scenarios where token theft (not connection spoofing) is the primary threat being mitigated.

**Interviewer follow-up:** *"If you already have short-lived access tokens, why bother with DPoP at all?"* — Short expiry limits the *window* of exposure but does nothing to prevent replay *within* that window; a stolen 5-minute token is still fully usable by an attacker for those 5 minutes from anywhere. DPoP doesn't replace short expiry — it's complementary defense-in-depth that makes the stolen token itself worthless without the client's private key, regardless of how long it's valid for.

...
11. **Where Runner Minutes Actually Go** / **Workflow-Level Timeout Hygiene** — cost/performance optimization source mein bilkul discuss nahi hui thi, aur "reduce our CI bill" ek common lead-style question hai.
12. **GitHub Actions vs Azure DevOps Pipelines** — is audience ke liye highly relevant (.NET shops frequently Azure DevOps run karti ya usse migrate karti hain); source mein present nahi tha.

**Contradictions/staleness flagged inline** (per se conflicting notes nahi, balki source mein factual staleness jo 2026 mein verbatim repeat karne par wrong hoti):
- Source mein throughout action versions (`checkout@v3`, `cache@v3`, `upload-artifact@v3`/`download-artifact@v3`) outdated/deprecated hain — is guide mein throughout v4+ par update ki gayi hain, Core Concepts aur Caching & Artifacts ke under ek explicit callout ke saath.
- Runner OS image list (`ubuntu-20.04`, `windows-2019`, `macos-12`) retired images ko reference karti hai — updated list provide ki gayi hai, GitHub ki live support matrix ke against verify karne ke liye flag ki gayi kyunki `-latest` rotate hota rehta hai.
- `actions/setup-debugging`, jise source mein debugging tool ke roop mein cite kiya gaya hai, ek real official action nahi lagta — source mein likely inaccurate flag kiya gaya, correct current approach (`ACTIONS_STEP_DEBUG`/`ACTIONS_RUNNER_DEBUG` secrets, `tmate`) substitute karke.

## Summary of `[gaps]` Additions (This Pass)

Ek follow-up gap-analysis review ne specifically flag kiya ki is guide ke cloud/deployment examples generic ya Azure-flavored patterns par default kar rahe the, bina AWS (candidate ka actual target cloud) ya Terraform/CDKTF (candidate ka actual IaC tooling) se tie back kiye. Ise close karne ke liye ek `[gaps]`-tagged section add kiya gaya:

1. **AWS-Native CI/CD Integration Points** — guide ne OIDC federation ko conceptually already cover kiya tha aur ek bare-bones `aws-actions/configure-aws-credentials` snippet dikhaya tha, lekin un AWS-specific mechanics par deep nahi gaya tha jo ek interviewer actually probe karega: full `configure-aws-credentials` flow (under the hood STS `AssumeRoleWithWebIdentity`), exact IAM OIDC provider + trust policy shape (ek Terraform `aws_iam_openid_connect_provider`/`aws_iam_role` example ke saath, kyunki Terraform candidate ka real IaC tool hai — sirf ek conceptual mention nahi), aur — wo part jo entirely missing tha — caching aur artifacts concretely **ECS** (image-based, ECR tak `type=gha` layer caching, task-definition rollout) versus **Lambda** (artifact/zip-based, deployable handoff ke roop mein `upload-artifact`) par deploy karne se kaise connect hote hain. Yeh matter karta hai kyunki akela "OIDC federation" ek general-cloud talking point hai; specific AWS action, trust-policy claim conditions (`aud`/`sub`), aur cache-vs-artifact mein se kaunsa kis AWS compute target ke liye fit hota hai — yeh name kar paana hi wo cheez hai jo ek interview mein AWS-specific (na ki generic-cloud) fluency ko actually signal karti hai.

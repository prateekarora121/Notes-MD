...
- Ek indexed column ko `WHERE` mein ek function/expression mein wrap karna (SARGability break karta hai, ek scan force karta hai).
- Yeh assume karna ki `TRUNCATE` ko kabhi roll back nahi kiya ja sakta (kiya ja sakta hai, ek explicit transaction ke andar — DELETE se asli difference logging granularity ka hai).
- Ek subquery ke against `NOT IN` use karna jo `NULL` contain kar sakti hai (silently zero rows return karta hai).
- Pre-2019 compatibility levels par ek table variable ke row estimate par trust karna (deferred compilation ke bina hamesha 1 row estimate hota hai).
- Yeh assume karna ki scalar UDFs hamesha slow hote hain — pre-2019 sach tha, aksar eligible functions ke liye compat level 150+ par Scalar UDF Inlining se fix ho jaata hai.
- Bhool jaana ki `LAST_VALUE()` ka default frame sirf current row tak peeche dekhta hai, poori partition tak nahi.
- Yeh maanna ki triggers per row fire hote hain — woh per statement fire hote hain; `inserted`/`deleted` bahut saari rows hold kar sakti hain.
- Over-indexing: har additional nonclustered index write cost add karta hai (insert/update/delete ko use maintain karna padta hai) — actual, observed query patterns ke liye index karo.
- Parameter sniffing ko ek "ek baar fix karne waale" bug ki tarah treat karna, bajaye ek ongoing trade-off ke jise per query actively manage karna hota hai.
- Ek plan mein `Key Lookup` operators ko ignore karna — volume par woh aksar ek scan se bhi bure hote hain; index ko covering banane ke liye `INCLUDE` columns add karo.
- Production par extended periods ke liye Profiler traces chalana — iske bajaye Extended Events use karo.
- Yeh bhool jaana ki foreign keys child column ko auto-index nahi karti, jiski wajah se har parent delete/update cascade check par scans hoti hain.

---

## Sample Interview Q&A (Answered)

**Q: Read Committed aur RCSI mein kya difference hai, aur aap ek existing production system mein RCSI kyun enable karoge?**
A: Default Read Committed short-lived shared locks use karta hai — readers writers ke peeche block ho sakte hain aur vice versa. RCSI wahi isolation *semantics* rakhta hai (abhi bhi non-repeatable reads allow karta hai, abhi bhi khud ko "Read Committed" kehta hai) lekin ise locking ke bajaye row versioning ke through implement karta hai, isliye readers bina block hue per statement last-committed version dekhte hain, aur writers readers se block nahi hote. Aap ise ek busy OLTP system mein reader/writer blocking eliminate karne ke liye enable karoge bina application code ko touch kiye (yeh ek database-level, transparent switch hai), tempdb version-store overhead ki cost par.

**Q: `ROW_NUMBER()`, `RANK()`, aur `DENSE_RANK()` same data par alag-alag results kyun return kar sakte hain, aur aap har ek ko kab choose karoge?**
A: Woh is baat par differ karte hain ki `ORDER BY` mein ties kaise handle hoti hain: `ROW_NUMBER()` har row ko ek unique number deta hai regardless of ties (ties ke beech arbitrary order) — jab aapko ek strictly unique sequence chahiye tab use karo (e.g., de-duplication, paging). `RANK()` tied rows ko same rank deta hai lekin subsequent numbers skip karta hai (1,2,2,4) — jab gaps ko tied competitors ki number reflect karni chahiye tab use karo. `DENSE_RANK()` tied rows ko same rank deta hai bina gaps ke (1,2,2,3) — "distinct rank tiers" reporting ke liye use karo (e.g., top 3 distinct price tiers, chahe multiple products ek price share karein).

**Q: Explain karo ki jab aap ek query ko pehli baar versus doosri baar run karte ho to end-to-end kya hota hai (plan caching).**
A: Pehla execution: parse → bind/algebrize → optimizer statistics-based cardinality estimates use karke candidate plans generate aur cost-compare karta hai → chosen plan query text ke ek hash se keyed hokar plan cache mein store hota hai (aur, parameterized queries ke liye, wahi cached plan alag parameter values ke across reuse ho sakta hai — yahi exactly wahaan hai jahaan parameter sniffing originate hota hai) → plan execute hota hai, results return hote hain. Same query text/parameterization ke saath doosra execution: SQL Server ek plan cache lookup karta hai aur, agar ek valid cached plan mil jaaye (koi recompile triggers nahi jaise schema change, threshold se beyond stats change, ya `SET` option differences), to use directly reuse karta hai, optimization step ko poori tarah skip karke — yahi main reason hai ki ek app mein bikhri hui ad-hoc, non-parameterized SQL expensive hoti hai: har thoda alag literal ek alag cache key aur ek fresh compile produce karta hai.

**Q: Ek stored procedure dev/QA mein fast chalti hai lekin production mein same query par slow — aap sabse pehle kya check karoge?**
A: Order mein: (1) data volume/distribution differences — production mein dev se zyada rows ya zyada skew hai; (2) parameter sniffing — check karo ki cached plan ek atypical parameter value ke against compile hua tha ya nahi, Query Store ya `sys.dm_exec_query_stats` use karke; (3) statistics freshness — agar `AUTO_UPDATE_STATISTICS` catch up na kar paaya ho to large loads ke baad production mein stale stats ho sakti hain; (4) ek environment mein present lekin doosre mein missing indexes (schema drift); (5) actual execution plans ko side-by-side compare karo, estimated-vs-actual row count mismatches par focus karke, jo seedhe (2) ya (3) ki taraf point karte hain.

**Q: Ek CTE aur ek temp table mein asli difference kya hai, "ek in-memory hai aur ek on disk" ke beyond?**
A: Ek CTE ek materialized object nahi hai — yeh macro-jaisa query text hai jo inline substitute hota hai (mostly; recursive CTEs iteratively execute hote hain lekin non-recursive CTEs typically optimizer dwara bas inline ho jaate hain, unless multiple times reference kiye jaayein jahaan kuch versions use spool kar sakte hain). Iski **apni koi statistics nahi** hoti aur ise index nahi kiya ja sakta. Ek temp table (`#t`) tempdb mein ek real, physical table hai jiski apni **statistics aur indexes** hoti hain, isliye optimizer iske baare mein properly cardinality-aware decisions le sakta hai, aur yeh same session mein multiple statements/batches ke across persist karta hai, jisse aap ek complex pipeline ko indexed intermediate steps mein break kar sakte ho. Rule of thumb: one-shot readability/recursion ke liye CTE; jab aapko ek intermediate result multiple times reuse karna ho ya use ek large row count ke liye indexed/statistics-backed chahiye tab temp table.

### Practical Query Challenges (Fully Solved)

Neeche diye gaye bare prompts original notes mein unanswered aaye the ("Level 1–10" practice list). Full solutions neeche.
...
Neeche di gayi sections original notes ke beyond add ki gayi hain, in-place **[new content]** tag ki gayi, kyunki ye senior/lead .NET interview level par commonly probe hoti hain aur source material mein missing ya sirf thinly covered thi:

- **OLTP vs OLAP** — foundational framing ki workload separation aur indexing strategy kyun differ karti hai; interviewers is vocabulary ki expect karte hain.
- **SARGability** — sabse common "yeh simple query slow kyun hai" root cause aur code-review finding; scattered notes mein implicit tha lekin kabhi named ya systematized nahi kiya gaya.
- **Index Seek vs Scan, Key Lookup, Covering Indexes** — "ek index add karo" advice ke peeche ki mechanics; Key Lookup cost ek frequent senior-level trap question hai.
- **Statistics & Cardinality Estimation** — explain karta hai ki plans *kyun* bure hote hain, sirf yeh nahi ki hote hain; CE version change aur ascending-key problem ko cover karta hai.
- **Parameter Sniffing** — trade-off ke hisaab se ranked fixes ke saath dedicated deep-dive; source mein sirf passing mein mention tha ("common in OLTP systems").
- **RCSI vs Snapshot Isolation** — source ne sirf "Snapshot" ko ek isolation-levels list mein ek bullet ke roop mein list kiya tha; yeh sabse zyada-poochhe jaane waale senior concurrency topics mein se ek hai aur ise full treatment ki zaroorat thi (enable mechanics, conflict detection differences, tempdb cost).
- **TempDB Contention & Configuration** — sirf ek one-line "Scenario 5" symptom/fix ke roop mein aaya; diagnostic queries aur file configuration guidance ke saath expand kiya gaya.
- **Query Store in Depth** — source mein ek short definition thi; force-plan syntax, capture-mode tuning, aur regression-detection workflow add kiya gaya.
- **Columnstore Indexes for Analytics** — source notes se entirely absent; modern SQL Server par reporting/analytics workloads ke baare mein kisi bhi conversation ke liye essential.
- **Table Partitioning** — source mein sirf 4 bullet points the; partition switching, elimination, aur "manageability vs performance" misconception correction ke saath expand kiya gaya.
- **Index Fragmentation & Maintenance** — fill factor aur REBUILD/REORGANIZE thresholds absent the; ek Performance Tuning section ke roop mein add kiya gaya.

**Contradiction flagged:** source notes multiple jagah kehti hain ki TRUNCATE "cannot be rolled back" as an absolute rule, jabki elsewhere yeh bhi correctly note karti hain ki DDL/DML ek explicit transaction ke andar roll back ho sakta hai. Yeh guide ise explicitly resolve karta hai: TRUNCATE ek explicit transaction ke andar rollback-capable **hai**; DELETE se asli, durable distinction *logging granularity* hai (page deallocation vs per-row logging), transactional capability nahi. Multiple duplicate/overlapping note sections ke beech koi doosra substantive factual contradiction nahi mila (e.g., `practice_views_updates.sql` content ki teen near-identical copies ko de-duplicate karke ek mein kar diya gaya).

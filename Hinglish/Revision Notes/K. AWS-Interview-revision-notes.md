...
Cross-checked section-by-section against the current guide (its 12-part structure). Two real gaps found and fixed in this update:
1. **Signal-based `input()`/`output()`/`model()`** (stable v19) — was missing from "Component Communication"; added with code, migration schematics, and the common `()`-call mistake.
2. **"Modern Angular Extras" (5 topics)** — `hostDirectives`, `afterRender`/`afterNextRender`, `NgOptimizedImage`, Router View Transitions, i18n — were entirely absent as a section (only glancingly referenced in old Q&A). Added as a full new section, plus their rapid-fire Q&A pairs.

Everything else in the guide (Core Concepts through Testing/Best Practices/Pitfalls/Version Table) was already represented here at matching or greater detail.

**Scope note — sections here that go beyond the current guide:** NgRx Entity Adapters/Facade Pattern/Selector Memoization, Dependency Injection Hierarchical deep-dive, Micro-Frontends/Module Federation, `@defer`, Lazy Loading/Preloading (own section), SSR/Angular Universal, Hydration+Event Replay, PWA/Service Workers, Angular Security (dedicated section), Accessibility (a11y), and Deployment (Firebase/GitHub Pages) exist in these notes but aren't in the current guide's scope. They're kept as-is since they're still valid, real interview topics for a senior/lead role — just flagging that they came from the earlier, broader notes rather than the current guide, in case that's a deliberate scope narrowing on your end.

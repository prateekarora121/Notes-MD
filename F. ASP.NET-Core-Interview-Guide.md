...
| **IIS** | Windows-only. Acts as a reverse proxy in front of Kestrel via the ASP.NET Core Module (ANCM), or in-process hosting where the app runs inside the IIS worker process itself (in-process is the default for IIS-hosted ASP.NET Core apps — see the in-process vs out-of-process detail below). Brings mature Windows-integrated hosting features (Windows Auth, IIS-level app pools, process recycling). |
